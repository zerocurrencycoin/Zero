...
3. **Implement Sapling Groth16 batch verification** (§6, full execution plan §9.4) — the largest bucket by far (48–55% of CPU post-Sapling-activation, §2). §6 scoped this: real batch-verification code exists in `zkcrypto/bellman` (the modern successor to Zero's pinned, ~2019-vintage `ebfull/bellman`) and the pinned `pairing::Engine::miller_loop` primitive it would need already exists in-tree, so a hand-port is feasible without a full crate-stack upgrade — but it requires restructuring `ContextualCheckBlock`'s per-transaction verify-or-reject control flow into buffer-then-batch-verify, plus a per-batch CSPRNG, and is real consensus-code-caliber work. §9.4 is a single ordered 23-step plan (7 phases: setup → pure-Rust correctness → FFI design → shadow-mode integration → controlled cutover → full validation → sign-off), grounded in the real FFI signatures in `depends/aarch64-apple-darwin25.3.0/include/librustzcash.h`, superseding §6's separate investigation/test lists. Not started — Phase 0 (fetch the pinned `librustzcash` source into a scratch checkout) is the next action.
...
- §9: status review across §5/§6, with a recommended sequencing (NEON before Groth16 — lower payoff, far lower risk, same vendoring/testing skills needed later). §9.2: NEON staged plan (vendor → standalone-KAT-harness → flag-gated wire-in → differential test → measure). §9.4: Groth16's containment strategy (§9.3) expanded into a single ordered 23-step, 7-phase execution plan (setup/fetch pinned source → pure-Rust correctness with zero consensus exposure → FFI boundary design → shadow-mode integration logging disagreements against the still-authoritative old path → flag-gated cutover with automatic fallback on batch failure → full validation → independent-review sign-off), grounded in the real `librustzcash.h` FFI signatures. Planning only, no implementation started for either.
...
1. Fetch the pinned source, not just headers/binaries: `zcash/librustzcash` at commit `06da3b9ac8f278e5d4ae13088cf0a4c03d2c13f5` (the exact pin in `depends/packages/librustzcash.mk`), into a scratch checkout — this is the actual source to edit, not the built artifacts already in `depends/`.
2. Confirm the FFI constraint above by reading `librustzcash/src/rustzcash.rs` in the fetched checkout: proof bytes arrive raw per-call. A batched entry point needs to either deserialize N times up front or accept an array of raw byte-arrays and deserialize inside the batch function itself.
3. In the fetched source, find exactly where `verify_proof` is called from `check_spend`/`check_output`'s Rust implementations; confirm `Proof`/`VerifyingKey`/`PreparedVerifyingKey` shapes against what `zkcrypto/bellman`'s `batch.rs` needs (fetch `zkcrypto/bellman` too, as a reference to port logic from — not a dependency to add).

**Phase 1 — Pure-Rust correctness, zero consensus exposure**

4. Write a `#[cfg(test)]`-only batch-verify function (scratch crate or a test module in the fetched checkout) using the pinned `bellman`/`pairing` types: per-proof random scalar `z_i`, weighted accumulation of `(A,B,C)` terms and public inputs, one call to the pinned `pairing::Engine::miller_loop` (already confirmed to accept the needed arbitrary-length iterator of prepared pairs) over the whole batch, one `final_exponentiation`.
5. Generate known-good/known-bad proof fixtures — the existing Sapling spend/output construction in `src/zcbenchmarks.cpp:706,739` is a reference for valid-proof shape, reachable from the Rust side via the same proving keys.
6. Test: for N = 1, 2, 8, 64, assert batch accept/reject exactly matches per-proof `verify_proof`, including N-1-bad-among-N cases.
7. **Exit criterion:** the standalone batch verifier agrees with per-proof `verify_proof` on every fixture, including adversarial ones. Do not proceed to Phase 2 with an open disagreement — a bug caught here is free; caught later, it isn't.
...
**Status: plan only, no implementation started.** Phase 0 (fetching the pinned `librustzcash` source into a scratch checkout) is the correct next action whenever this is picked up.
