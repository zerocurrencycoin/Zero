...
- §9: status review across §5/§6, with a recommended sequencing (NEON before Groth16 — lower payoff, far lower risk, same vendoring/testing skills needed later). §9.2: NEON staged plan (vendor → standalone-KAT-harness → flag-gated wire-in → differential test → measure) — planning only, not started. §9.4: Groth16's containment strategy (§9.3) expanded into a single ordered 23-step, 7-phase execution plan; **Phases 0–1 executed and passing this session** — pinned `librustzcash`/`bellman`/`pairing` source fetched (outside this repo), a hand-ported random-linear-combination batch verifier built and tested against the pinned crate's real MiMC/BLS12-381 test circuit, zero disagreements with per-proof `verify_proof` across N=1,2,8,64 and adversarial one-corrupted-proof cases, repeated 6 runs; also confirmed the pinned 2018-era crate pair builds clean under a modern Rust 1.90 toolchain (an open risk, now resolved), and refined the FFI design (Phase 2 item 8) with the finding that `check_spend`/`check_output` do signature/small-order checks before proof verification, so only the `verify_proof` step itself batches. Phase 2 onward (build/link changes, then `main.cpp` consensus-code edits) intentionally not started pending explicit go-ahead.
