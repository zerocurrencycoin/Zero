...
1. **NEON blake2b integration for Equihash verification** (§5, recommended path §9.2) — a maintained implementation exists at [`BLAKE2/BLAKE2`](https://github.com/BLAKE2/BLAKE2/tree/master/neon)'s `neon/` directory (`blake2b-neon.c` + a dedicated `Aarch64` makefile; most recent commit 2023, by `veorq`/Samuel Neves, one of the two original BLAKE2 authors). §9.2 lays out a staged, low-risk integration plan (vendor → standalone KAT harness → flag-gated wire-in → differential test → measure). **Recommended as the next item to actually start** — lower payoff than Groth16 batching but far lower risk (not consensus-critical), and exercises the same vendoring/testing muscle the Groth16 work needs. First sub-step: confirm whether this matters for real deployments (ARM vs. x86_64 node population) before investing further.
2. **Code and test: `-O1` vs `-O2` build comparison** — a user-reported "little difference," never reproduced with a real measurement.
3. **Implement Sapling Groth16 batch verification** (§6, recommended path §9.3) — the largest bucket by far (48–55% of CPU post-Sapling-activation, §2). §6 scoped this: real batch-verification code exists in `zkcrypto/bellman` (the modern successor to Zero's pinned, ~2019-vintage `ebfull/bellman`) and the pinned `pairing::Engine::miller_loop` primitive it would need already exists in-tree, so a hand-port is feasible without a full crate-stack upgrade — but it requires restructuring `ContextualCheckBlock`'s per-transaction verify-or-reject control flow into buffer-then-batch-verify, plus a per-batch CSPRNG, and is real consensus-code-caliber work. §6 has a 6-step investigation plan and 7-step test plan; §9.3 adds a 5-stage containment strategy (pure-Rust correctness → shadow-mode differential testing against real chain data with the old path still authoritative → flag-gated cutover with automatic fallback on batch failure → full test plan → independent review) specifically because the failure mode here (silently accepting an invalid shielded proof) is categorically worse than anything else this investigation has touched. Not started past scoping/planning.
4. **Size and consider removing/gating `CBlockIndex`'s always-present "Shieldex" fields** (§8.2) — 22 `int64_t` fields (~176 bytes/block, ~435MB chain-wide at tip) exist in every `CBlockIndex` instance regardless of whether `-zindex` is enabled; disk serialization and population are correctly gated on `fZindex`, the in-memory struct layout is not. One field (`nNotarizations`) is confirmed fully dead (never incremented outside a commented-out heuristic). Not sized against `AddToBlockIndex`'s overall footprint or scoped for a fix.
...
- §6: scoped Sapling Groth16 batch-verification headroom — confirmed no batching exists in the pinned `bellman`, confirmed the underlying `miller_loop` primitive needed to hand-port it already does, confirmed the `zcash-scriptch` idle-thread angle is a queue-wiring gap not a proof-system limitation. Scoping done, concrete investigation/implementation/test plan written; no implementation yet (item 3 above).
- §7: memory profiling via `vmmap`/`heap`/`malloc_history` (CLI-native, unblocking the Instruments GUI-only-export dead end), full chain sweep to height 2,470,587 (chain tip) — found footprint grows with chain length (no leak signature), `AddToBlockIndex`'s permanent block-index map is the single largest allocation site (~66% of tracked `ThreadImport` allocation in the sampled window), and confirmed Sapling Groth16 verification allocates essentially zero heap despite dominating CPU. Also caught and corrected a real methodology trap: macOS memory compression makes `vmmap`'s headline `Physical footprint` figure misleading for cross-checkpoint comparison over a multi-hour run (it showed a spurious "growth rate is slowing down" pattern) — `Writable regions: Total` is the figure that isn't confounded by compression.
- §8: broke `AddToBlockIndex`'s aggregate allocation figure down to its 4 actual per-block call sites (counts, sizes, lifetimes); reviewed the `CBlockIndex` "Shieldex" stat fields end-to-end (real RPC consumer confirmed, disk/population correctly gated on `-zindex`, in-memory layout not gated, one field — `nNotarizations` — confirmed fully dead); resolved an unattributed Rust `finish_grow` allocator symbol to `bellman::groth16::Parameters::read`, i.e. the same one-time startup key-loading cost §7 had already found, not a new or per-block cost; confirmed the always-active caches (coins/nullifier/anchor) are consensus-load-bearing for every node configuration, not over-built.
- §9: status review across §5/§6, with a recommended sequencing (NEON before Groth16 — lower payoff, far lower risk, same vendoring/testing skills needed later) and a concrete staged/containment plan for each: NEON via vendor→standalone-KAT-harness→flag-gated wire-in→differential test (§9.2); Groth16 via a 5-stage pure-Rust→shadow-mode→flag-gated-cutover-with-fallback→full-test-plan→independent-review containment strategy specifically because a batching bug's worst failure mode (silently accepting an invalid shielded proof) has no equivalent in this investigation's other work (§9.3). Planning only, no implementation.
...
**Concrete plan for a real implementation, if picked up (still not started — this is a plan, not code).**
...
§6 already has a 6-step investigation plan and 7-step test plan (added this session). The additional recommendation here is about **sequencing and containment** — how to make consensus-critical work safe to iterate on, not just what steps exist.

**Core principle: never let the batched path be the only path.** Every stage below keeps the existing, proven single-proof `verify_proof` call as a mandatory fallback or cross-check, so a bug in the new code can only cause *extra* verification work, never a wrong accept/reject — until the very last, explicitly-flagged stage.

1. **Stage 0 — pure-Rust correctness, zero consensus exposure.** Investigation steps 1–2 from §6 (confirm struct-shape compatibility, prototype the batch math as a `#[cfg(test)]`-only unit against known-answer proof fixtures). Nothing in this stage touches the FFI boundary or `main.cpp` — a bug here cannot possibly affect a running node, by construction. This is where the bulk of "does the hand-ported math actually work" risk should be retired.
2. **Stage 1 — shadow-mode FFI integration: batch-verify runs, but never decides anything.** Add the new FFI entry point(s) (investigation step 3), but wire them in *alongside* the existing per-proof calls, not in place of them: for every block, run both the existing sequential `verify_proof` loop (authoritative, decides accept/reject as today) **and** the new batch path (informational only), and log any disagreement between them as a loud error — without ever acting on the batch path's result. This turns every real `-reindex`/sync run during development into a free, continuous differential test against real chain data, with zero risk of a wrong consensus decision, since the old code path is still the one deciding. Keep this stage running for a meaningful stretch of real reindex/sync activity (e.g. the same height windows already used in §2/§3, spanning both pre- and post-Sapling) before trusting the batch path's correctness.
3. **Stage 2 — controlled cutover behind a flag, with automatic fallback on batch failure.** Only after Stage 1 shows zero disagreements across a large real-data sample: make the batch path authoritative, but *only* for the accept case — if the batch verify reports failure, fall back to the sequential per-proof path (via `verify_single`, per §6's existing plan) to get the real, individually-attributed error before rejecting the block. This means a bug that causes the batch path to *wrongly reject* a valid block is self-correcting (the fallback re-checks and would accept it — worth an explicit test for this exact scenario), while a bug that causes it to *wrongly accept* an invalid proof is the one failure mode that has no safety net at this stage — which is exactly why Stage 1's disagreement-logging needs to have run clean first.
4. **Stage 3 — full test plan from §6** (known-answer vectors, adversarial corrupted-proof tests, full Boost/gtest regression, real-chain differential `chainstate`-hash comparison, perf re-measurement, optionally multicore scaling) — run against the Stage 2 build, not earlier, since this is the first stage where the code under test matches what would actually ship.
5. **Independent review gate before removing the shadow/fallback safety net.** Given the failure mode identified in the status table (§9.1: a wrong accept is a silently-accepted invalid shielded spend, much worse than a crash or an incorrect reject), get a second implementer or reviewer to independently re-derive the batching math against the published random-linear-combination technique and the pinned crate's actual types — not just review the diff — before this is considered ready to run as the sole verification path with no fallback.

**Why this is more work than "port `batch.rs` and test it," and worth it anyway:** the failure mode being guarded against — a false-accept of an invalid shielded proof — is categorically worse than anything else in this investigation has touched (§3/§4's fixes were pure memoization with no semantic change; this one isn't). Stages 1–2 turn every day of ordinary development/testing activity into free differential-testing signal against real chain data before the new path is ever trusted to decide anything alone, which is a substantially stronger validation posture than a fixed test suite alone can provide for a change of this kind.
