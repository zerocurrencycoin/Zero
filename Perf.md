...
**Footprint grows with chain length, roughly linearly, not unboundedly — no leak signature found.** Two checkpoints so far on the full-chain sweep: 535.3MB at height 278,072, 956.1MB at height 500,436 (spans Sapling activation at 492,850) — a rate of ~1.9KB/block over that span. This is consistent with §0's next-step framing (was there unbounded growth?) resolving to "no": the growth tracks `AddToBlockIndex` building a permanent, one-entry-per-block-header in-memory index (see below), which is expected, bounded-per-block, and doesn't compact or evict, not a leak. (Chain-length-proportional growth is real and worth knowing operator-side — a fully-synced node's block-index memory floor scales with chain height — but that's a capacity-planning fact, not a defect.)
