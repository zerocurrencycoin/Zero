...
- [ ] **`CZeronodeBroadcast::CheckInputsAndAdd`** (**`src/zeronode/zeronode.cpp`**): **`pConfIndex`** from **`chainActive[...]`** is now null-checked (defer / erase broadcast). **`CheckAndVerify`** and any other **`chainActive[height]`** sites should be audited the same way (**UpdateZero.md** §8).
