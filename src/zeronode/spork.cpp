...
// Zero: on startup load spork values from previous session if they exist in the sporkDB
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_6_ZERONODE_FULL_PAYMENT_ENABLED) r = SPORK_6_ZERONODE_FULL_PAYMENT_ENABLED_DEFAULT;
        if (nSporkID == SPORK_7_ZERONODE_PAYMENT_ENABLED) r = SPORK_7_ZERONODE_PAYMENT_ENABLED_DEFAULT;
        if (nSporkID == SPORK_8_ZERONODE_PAYMENT_ENFORCEMENT) r = SPORK_8_ZERONODE_PAYMENT_ENFORCEMENT_DEFAULT;
        if (nSporkID == SPORK_9_ZERONODE_BUDGET_ENFORCEMENT) r = SPORK_9_ZERONODE_BUDGET_ENFORCEMENT_DEFAULT;
...
bool CSporkManager::Sign(CSporkMessage& spork)
{
    ...
    if (!obfuScationSigner.SignMessage(strMessage, errorMessage, spork.vchSig, key2)) {
        LogPrintf("CZeronodePayments::Sign - Sign message failed");
        return false;
    }

    if (!obfuScationSigner.VerifyMessage(pubkey2, spork.vchSig, strMessage, errorMessage)) {
        LogPrintf("CZeronodePayments::Sign - Verify message failed");
        return false;
    }
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if (strName == "SPORK_6_ZERONODE_FULL_PAYMENT_ENABLED") return SPORK_6_ZERONODE_FULL_PAYMENT_ENABLED;
    if (strName == "SPORK_7_ZERONODE_PAYMENT_ENABLED") return SPORK_7_ZERONODE_PAYMENT_ENABLED;
    if (strName == "SPORK_8_ZERONODE_PAYMENT_ENFORCEMENT") return SPORK_8_ZERONODE_PAYMENT_ENFORCEMENT;
    if (strName == "SPORK_9_ZERONODE_BUDGET_ENFORCEMENT") return SPORK_9_ZERONODE_BUDGET_ENFORCEMENT;
...
std::string CSporkManager::GetSporkNameByID(int id)
{
    ...
    if (id == SPORK_6_ZERONODE_FULL_PAYMENT_ENABLED) return "SPORK_6_ZERONODE_FULL_PAYMENT_ENABLED";
    if (id == SPORK_7_ZERONODE_PAYMENT_ENABLED) return "SPORK_7_ZERONODE_PAYMENT_ENABLED";
    if (id == SPORK_8_ZERONODE_PAYMENT_ENFORCEMENT) return "SPORK_8_ZERONODE_PAYMENT_ENFORCEMENT";
    if (id == SPORK_9_ZERONODE_BUDGET_ENFORCEMENT) return "SPORK_9_ZERONODE_BUDGET_ENFORCEMENT";
