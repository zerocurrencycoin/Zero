...
#ifndef ZERONODE_BUDGET_H
#define ZERONODE_BUDGET_H
...
//
// CBudgetVote - Allow a zeronode node to vote and broadcast throughout the network
//
...
//
// CFinalizedBudgetVote - Allow a zeronode node to vote and broadcast throughout the network
//
...
public:
    ...
    std::map<uint256, CBudgetProposalBroadcast> mapSeenZeronodeBudgetProposals;
    std::map<uint256, CBudgetVote> mapSeenZeronodeBudgetVotes;
    std::map<uint256, CBudgetVote> mapOrphanZeronodeBudgetVotes;
    ...
    void ClearSeen()
    {
        mapSeenZeronodeBudgetProposals.clear();
        mapSeenZeronodeBudgetVotes.clear();
    ...
    void Clear()
    {
        ...
        mapSeenFinalizedBudgets.clear();
        mapSeenFinalizedBudgetVotes.clear();
        mapOrphanZeronodeBudgetVotes.clear();
    ...
    inline void SerializationOp(Stream& s, Operation ser_action)
    {
        READWRITE(mapSeenZeronodeBudgetProposals);
        READWRITE(mapSeenZeronodeBudgetVotes);
        READWRITE(mapSeenFinalizedBudgets);
        READWRITE(mapSeenFinalizedBudgetVotes);
        READWRITE(mapOrphanZeronodeBudgetVotes);
...
private:
    // critical section to protect the inner data structures
    mutable CCriticalSection cs;
    bool fAutoChecked; //If it matches what we see, we'll auto vote for it (zeronode only)
...
public:
    ...
    void AutoCheck();
    //total zero paid out by this budget
    CAmount GetTotalPayout();
    //vote on this finalized budget as a zeronode
    void SubmitVote();
...
//
// Budget Proposal : Contains the zeronode votes for each budget
//
