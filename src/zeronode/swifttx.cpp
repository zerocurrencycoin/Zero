...
//step 2.) Top SWIFTTX_SIGNATURES_TOTAL zeronodes, open connect to top 1 zeronode.
//         Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 zeronode, waits for SWIFTTX_SIGNATURES_REQUIRED messages. Upon success, sends "txlock'
        ...
        if (ProcessConsensusVote(pfrom, ctx)) {
            ...
            */
            if (!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)) {
                if (!mapUnknownVotes.count(ctx.vinZeronode.prevout.hash)) {
                    mapUnknownVotes[ctx.vinZeronode.prevout.hash] = GetTime() + (60 * 10);
                }

                if (mapUnknownVotes[ctx.vinZeronode.prevout.hash] > GetTime() &&
                    mapUnknownVotes[ctx.vinZeronode.prevout.hash] - GetAverageVoteTime() > 60 * 10) {
                    LogPrintf("ProcessMessageSwiftTX::ix - zeronode is spamming transaction votes: %s %s\n",
                        ctx.vinZeronode.ToString().c_str(),
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = znodeman.GetZeronodeRank(activeZeronode.vin, nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

    if (n == -1) {
        LogPrint("swiftx", "SwiftX::DoConsensusVote - Unknown Zeronode\n");
        return;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("swiftx", "SwiftX::DoConsensusVote - Zeronode not in the top %d (%d)\n", SWIFTTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    CConsensusVote ctx;
    ctx.vinZeronode = activeZeronode.vin;
...
{
    int n = znodeman.GetZeronodeRank(ctx.vinZeronode, ctx.nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

    CZeronode* pzn = znodeman.Find(ctx.vinZeronode);
    if (pzn != NULL)
        LogPrint("swiftx", "SwiftX::ProcessConsensusVote - Zeronode ADDR %s %d\n", pzn->addr.ToString().c_str(), n);

    if (n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("swiftx", "SwiftX::ProcessConsensusVote - Unknown Zeronode\n");
        znodeman.AskForZN(pnode, ctx.vinZeronode);
        return false;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("swiftx", "SwiftX::ProcessConsensusVote - Zeronode not in the top %d (%d) - %s\n", SWIFTTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if (!ctx.SignatureValid()) {
        LogPrintf("SwiftX::ProcessConsensusVote - Signature invalid\n");
        // don't ban, it could just be a non-synced zeronode
        znodeman.AskForZN(pnode, ctx.vinZeronode);
        return false;
    }
    ...
    if (i != mapTxLocks.end()) {
        ...
        if (pwalletMain) {
            ...
            if (g_zeronodeWallet && g_zeronodeWallet->GetRequestCount(ctx.txHash) > 0)
                g_zeronodeWallet->IncrementRequestCount(ctx.txHash);
        }
#endif
        ...
        if ((*i).second.CountSignatures() >= SWIFTTX_SIGNATURES_REQUIRED) {
            ...
            if (!CheckForConflictingLocks(tx)) {
                ...
                if (g_zeronodeWallet && g_zeronodeWallet->IsAvailable()) {
                    if (g_zeronodeWallet->UpdatedTransaction((*i).second.txHash)) {
                        nCompleteTXLocks++;
                    }
                }
#endif
...
{
    arith_uint256 temp = (UintToArith256)(vinZeronode.prevout.hash) + vinZeronode.prevout.n;
...
bool CConsensusVote::SignatureValid()
{
    ...
    if (pzn == NULL) {
        LogPrintf("SwiftX::CConsensusVote::SignatureValid() - Unknown Zeronode\n");
        return false;
    }

    if (!obfuScationSigner.VerifyMessage(pzn->pubKeyZeronode, vchZeroNodeSignature, strMessage, errorMessage)) {
...
bool CConsensusVote::Sign()
{
    ...
    //LogPrintf("signing privkey %s \n", strZeroNodePrivKey.c_str());

    if (!obfuScationSigner.SetKey(strZeroNodePrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CConsensusVote::Sign() - ERROR: Invalid zeronodeprivkey: '%s'\n", errorMessage.c_str());
        return false;
    }

    if (!obfuScationSigner.SignMessage(strMessage, errorMessage, vchZeroNodeSignature, key2)) {
        LogPrintf("CConsensusVote::Sign() - Sign message failed");
        return false;
    }

    if (!obfuScationSigner.VerifyMessage(pubkey2, vchZeroNodeSignature, strMessage, errorMessage)) {
...
bool CTransactionLock::SignaturesValid()
{
    BOOST_FOREACH (CConsensusVote vote, vecConsensusVotes) {
        int n = znodeman.GetZeronodeRank(vote.vinZeronode, vote.nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

        if (n == -1) {
            LogPrintf("CTransactionLock::SignaturesValid() - Unknown Zeronode\n");
            return false;
        }

        if (n > SWIFTTX_SIGNATURES_TOTAL) {
            LogPrintf("CTransactionLock::SignaturesValid() - Zeronode not in the top %s\n", SWIFTTX_SIGNATURES_TOTAL);
            return false;
        }
