...
/** Open a KAT file from repo root or contrib/perf/kats/ (cwd: root, src/, src/test/). */
static bool OpenKatFile(const char* basename, std::ifstream& in)
{
    const std::string candidates[] = {
        basename,
        std::string("contrib/perf/kats/") + basename,
        std::string("../") + basename,
        std::string("../contrib/perf/kats/") + basename,
        std::string("../../") + basename,
        std::string("../../contrib/perf/kats/") + basename,
        std::string("src/../") + basename,
        std::string("src/../contrib/perf/kats/") + basename,
    };
...
BOOST_AUTO_TEST_CASE(dump_mainnet_genesis_192_7_indices) {
    ...
    out << "# Source: Params().GenesisBlock() header (I = CEquihashInput||nNonce; not string-I TestEquihashSolvers form)\n";
