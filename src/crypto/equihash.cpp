...
int Equihash<N,K>::InitialiseState(EhHashState& base_state)
{
    ...
    unsigned char personalization[UB_PERSONALBYTES] = {};
                  ...
                  unsigned char* hash, size_t hLen)
{
    ...
    // it and writes the digest, which is the whole point of the prefix layer.
    eh_index lei = htole32(g);
    ub_hash_tail(base_state.get(), &lei, sizeof(eh_index), hash, hLen);
}
