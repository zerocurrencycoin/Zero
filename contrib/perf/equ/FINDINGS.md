# Equihash (192,7) -- current findings

**Subject: what is measured and computed about Zero's Equihash at production
parameters**, and the size of the gap to competitive mining.

**Inclusion rule.** A number or result belongs here. How the solver represents
data belongs in **`SOLVER.md`**; which solver runs and where it came from
belongs in **`VENDORED.md`**; what to build belongs in **`PLAN.md`**; how to
validate belongs in **`METHOD.md`**.

Every number marked `[Measured]` comes from this tree. `[Computed]` is derived
from source constants and is reproducible from them. Anything else is labelled
as an estimate.
...
**Zero and tromp made different trades, not better and worse ones:**
...
`mine_bench.sh` provides `regtest`, `mainnet-template` and a symbol-probe mode.
...
- **arm64 vs x86-64 matters more here than anywhere else**, because the
  candidate optimizations are SIMD intrinsics. A result on one ISA says nothing about
