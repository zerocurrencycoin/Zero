...
| Reindex / bootstrap / catch-up throughput | M-RX-TINY | `wall_s`, `height_per_s` | Tip **187417**; **198 s**; **946.6 h/s** | `campaign` | `none` | `AtHeight.md` (2026-07 manual) |
| Reindex / bootstrap / catch-up throughput | M-RX-TINY-20260811a | `wall_s`, `height_per_s` | Tip **187417**; **193 s**; **971.1 h/s**; `run_id=tiny-20260811T081310Z` | `campaign` | `debug_log` | `test-logs/measures_tiny-20260811T081310Z.csv` |
...
| Reindex / bootstrap / catch-up throughput | M-RX-SHORT-DELTA | marginal 3rd blk | **76 s**; **770.7 h/s** (from 2026-07 tiny/short) | `estimate` | `none` | `AtHeight.md` derived |
| Reindex / bootstrap / catch-up throughput | M-RX-SHORT-DELTA-20260811 | marginal 3rd blk | **59 s**; **992.8 h/s** (247-188 wall; 58575 ht) | `estimate` | `debug_log` | derived from 20260811b tiny/short |
| Reindex / bootstrap / catch-up throughput | M-RX-PHASEA | partial wipe reindex | **~190 s** to ht **~198k**; **~1053 h/s** sample; RSS **~523-540 MiB** | `campaign` | `lab_monitor` | out-of-tree bench REPORT |
...
| Reindex / bootstrap / catch-up throughput | M-RX-ONSET | stock reindex Sapling-onset | **140.19 blk/s** (n=1); window 490k-520k; ledger `CAMPAIGN=reindex-onset`; peer M-BOOT-ONSET (~parity; both ~2x slower than deep post-Sap ~300) | `campaign` | `lab_monitor` | `REPORT-reindex-onset.md` |
| Reindex / bootstrap / catch-up throughput | M-LAB-WALL-SECONDS | tiny reindex harness | **Superseded 2026-09-07 by M-LAB-WALL-MS.** Log-derived wall time was whole seconds, so the rate was discrete at ~9.8 blk/s (0.7%) | `spot` | computed | `test-logs/fullrun-20260907T071404Z/ANALYSIS.md` |
| Reindex / bootstrap / catch-up throughput | M-LAB-POLL-BACKOFF | tiny reindex, backoff pacing | 5 s far / 2 s within 10000 blocks of target: **64 polls -> 28**, span 141.6 -> 136.4 s. 7.6 s of overhead removed, 2.4 s traded back as detection latency, **net 5.2 s**. Polling falls from 9.6% to 4.4% of the span | `paired` | `tiny_baseline.sh` | `test-logs/tiny-20260908T192830Z-progress.tsv` |
| Reindex / bootstrap / catch-up throughput | M-LAB-POLL-COST | tiny reindex, progress series | Each `getblockcount` poll costs **212 ms** (RPC + process spawn) against a nominal 2 s sleep: 64 polls consume **13.6 s = 9.6%** of a 141.8 s run, inside the timed span | `series` | `tiny_baseline.sh` progress.tsv | `test-logs/tiny-20260908T181122Z-progress.tsv` |
| Reindex / bootstrap / catch-up throughput | M-LAB-BAND-TINY | tiny reindex, progress series | Rate varies by height band within one run: **1383 / 1417 / 1392 / 1354 / 1301 / 1253 / 1361 / 1105 blk/s** across 25k bands 0-187417. Mean 1322; the single-figure rate hides a 28% spread | `series` | `tiny_baseline.sh` progress.tsv | `test-logs/tiny-20260908T181122Z-progress.tsv` |
| Reindex / bootstrap / catch-up throughput | M-LAB-REPRO | tiny reindex, ms timing | Two runs 8 ms apart over 141 s: **1328.087 vs 1328.012 blk/s, 0.0057%**. **n=2, one workload, one machine, both idle.** Not a general variance figure: an unquiesced machine moved the same benchmark 15% (2026-09-05), and no other workload has been repeated at this resolution | `paired` | `tiny_baseline.sh` | `test-logs/tiny-20260907T083637Z-driver.log` |
| Reindex / bootstrap / catch-up throughput | M-LAB-WALL-MS | tiny reindex harness | Launcher times the measured span directly: **141126 ms -> 1328.011848 blk/s**. Resolution 1 ms = 0.0094 blk/s, a **1000x** improvement on the log-derived increment | `spot` | `tiny_baseline.sh` | `test-logs/tiny-20260907T083004Z-driver.log` |
| Reindex / bootstrap / catch-up throughput | M-EQ-ROW-WIDTH | `EhOptimisedSolve` (192,7) | `TruncatedWidth` = **70 B**; rounds 0-3 need 22-25 B, so a ~3x overcharge on the rounds holding the most rows | `spot` | source + computed | `equ/SOLVER.md` |
...
| Reindex / bootstrap / catch-up throughput | *(none yet)* | mainnet (192,7) timed solve | **Scheduled -- Track M / G5** -- `MINE_MAINNET_SOLVE=1` + Instruments; harness stub only (`mine_bench.sh mainnet-template`) | -- | -- | Perf §0.9 / §0.16 |
| Reindex / bootstrap / catch-up throughput | M-WAL-SYNC-P0 | wallet profile0 + tiny `-reindex` | tip **187417** in ~198 s (~**950** blk/s class); RSS **104->408 MiB**; wallet **106496** B flat; txcount **0** | `campaign` | `lab_monitor` | `test-logs/walletsync-20260812T153358Z/util.tsv` |
| Reindex / bootstrap / catch-up throughput | M-WAL-SYNC-P1 | wallet profile1 + tiny `-reindex` | tip **187417** in **~201 s** (~**918** blk/s from h~2.8k); RSS **~103->398 MiB**; wallet **237568** B flat; txcount **133**; note_tx **0** -- near P0; no witness hotspot expected | `campaign` | `lab_monitor` | `test-logs/walletsync-20260813T055703Z/` |
| Reindex / bootstrap / catch-up throughput | M-WAL-SYNC-FAT | fat wallet + tiny `-reindex` | tip **187417** in **~2.75 h** (~**19 blk/s**); wallet **785457152** B flat; txcount **801619**; RSS ~0.7->1.5 GiB -- **~50x** slower than M-WAL-SYNC-P0; util.tsv stalled mid-run (getwalletinfo vs cs_wallet) -- wall from debug.log tip + mid-run captures | `campaign` | `lab_monitor` + `xctrace` | `test-logs/walletsync-20260812T174850Z/`; CPU `test-logs/walletsync-fat-cpu-20260812T194107Z/` + `archives/...tar.gz` |
| Reindex / bootstrap / catch-up throughput | M-CPU-WAL-FAT | fat wallet mid-sync Time Profiler | 5 sync windows h **133k-181k**: loadblk **~99.7%**; after G0b needles `witness_cache` **~97%**, `wallet_add_ordered` **~0.03%**; tip idle. blk/s **~20-21** flat | `campaign` | `xctrace` + `bucket_profile.py` | `test-logs/walletsync-fat-cpu-20260812T194107Z/`; rebucket `capture_005/buckets_ALL_g0b.txt` |
| Reindex / bootstrap / catch-up throughput | M-WAL-NOTE-DENS | fat golden note density | **note_tx 1403 / txcount 801619 (0.175%)**; sprout **0**; sapling **1403** | `spot` | `cli` getwalletinfo | `test-logs/g0c-note-density/` |
| Reindex / bootstrap / catch-up throughput | M-WAL-WITNESS-IBD-AB | `-walletwitness=ibd-defer` A/B | fat tiny to h~15k: stock **16.75** vs defer **595** blk/s (~**35x**) | `campaign` | `lab_monitor` | `test-logs/g0d-witness-ibd-ab/SUMMARY.txt` |
| Reindex / bootstrap / catch-up throughput | M-WAL-WITNESS-NOTEIDX-AB | `-walletwitnessnote=1` A/B | fat tiny to h~8k: stock **14.86** vs noteidx **485.7** blk/s (~**33x**); no ibd-defer | `campaign` | `lab_monitor` | `test-logs/g0-noteidx-ab/SUMMARY.txt` |
| Reindex / bootstrap / catch-up throughput | M-WAL-DIRTY-CONT | INV-DIRTY-CONT stock+NOTEIDX+stats | tiny to h~11k: `scan_txs=1403`/`mapWallet=801619`; **note_visits=0** (all notes Sapling, tiny pre-Sap) -- early_continue **N/A** on this band | `spot` | `lab_monitor` | `test-logs/witness-lab-dirty-cont-20260813T041447Z/` |
| Reindex / bootstrap / catch-up throughput | M-WAL-WITNESS-REBUILD | BENCH-WIT-REBUILD defer +/- NOTEIDX | tiny tip **187417**: import+defer wall ~**333 s** either flag; tip `RebuildWitnessCacheForChainTip` runs but **height walk skipped** (no notes at/below tip) -- walk perf needs post-Sap tip; e2e validates walk with notes | `campaign` | `lab_monitor` | `test-logs/witness-lab-rebuild*-20260813T*/` |
| Reindex / bootstrap / catch-up throughput | M-WAL-WITNESS-TIP-AB | tip-only fat WIT height-walk +/- NOTEIDX | tip **2518018**; rescan window startHeight **2516577** (~1441 blk): stock `scan_txs=801619` **7659 ms** vs note `scan_txs=1403` **220 ms** (~**35x**); `note_tx_count=1403`. Walk runs at end of wallet Rescan; `-walletwitness=rebuild` hook after. Requires rebuilt binary with `-walletwitnessnote` | `campaign` | `lab_monitor` | `test-logs/witness-lab-tip-rebuild-*-20260813T071*` |
...
| Cache / dbcache / RPC micro-latency | M-CACHE-MATRIX | budgets + p50 | e.g. dbcache **800** off: **100/183/517** MiB; generate(100) p50 **~835 ms**; getdbinfo **~4.3 ms** | `repro` | `cli_timer` + `debug_log` | `contrib/perf/measure_dbcache_utxo.py`; ZeroStruct §4.3.4; TEST_ZERO |
| Cache / dbcache / RPC micro-latency | M-CACHE-2500 | longer mine | peak UTXO **0.1 MiB / 500** entries; tip txouts **3000** | `campaign` | same | TEST_ZERO sample `20260725_193336` |
| Cache / dbcache / RPC micro-latency | M-CACHE-MAIN | insight mainnet spot (`env=insight`) | BI/CS/UTXO **600/58/142**; tip cache **~77 MiB**; do not apply to non-insight wallet hosts | `spot` | `debug_log` | ZeroStruct §4.3.1 |
...
| Witness rebuild | M-WIT-LOG | Building Witnesses progress | **No archived wall_s** | `capability` | `debug_log` | `wallet.cpp` log line |
| Witness rebuild | M-WIT-EST | proposed shielded reindex effort | **~5-20 min/run** (estimate only) | `estimate` / `plan` | `none` | ExtTests / WitnessReindex notes |

**Gap:** highest-value missing duration class for status/ops tooling.

| getalldata / wallet RPC | M-GAD-SOFT | coalesce gate | default **-rpcdatacontinue=20** s | `repro` | `none` | node help / DevFee README |
| getalldata / wallet RPC | M-GAD-HOT | fat-wallet profile | hot in `EncodeBase58Check`; **no wall_ms table** | `spot` | sample/profile | ZeroStruct §6.2; DevFee README |
| getalldata / wallet RPC | M-GAD-S7 | post-S7 expectation | "lower user time"; unquantified | `plan` | `cli_timer` | DevFee README |

| Shield / drain (ops) | M-SH-300400 | shield UTXO limit | **300:** 10/10 ok ~90 KB; **400:** 7/10 ok ~119 KB | `campaign` | `ops_probe` | out-of-tree shield probe log |
| Shield / drain (ops) | M-DR-AGE | shield conf age | e.g. n=148; p50 **120 s**; p90 **330 s**; max **886 s** | `campaign` | `ops_probe` | `drain_idx_*.log` (sample 20260805) |

Use case: ops reliability under tip quiet; not consensus validation cost.

| Harness / test suite walls | M-H-STRICT | `--strict` | **~211-212 s** | `spot`/`repro` | `none` | TEST_ZERO |
| Harness / test suite walls | M-H-ALL | `--all` | **~1063 s** (2026-07-02); stale **~1275 s** also listed | `campaign` | `none` | TEST_ZERO |
| Harness / test suite walls | M-H-SUITE | `--suite` | **~1306 s** | `spot` | `none` | TEST_ZERO |
| Harness / test suite walls | M-H-RPC-OUT | slow scripts | protectcoinbase **220 s**; zkey_import **108 s**; ... | `campaign` | `none` | TEST_ZERO §3 |
| Harness / test suite walls | M-H-WB | walletbackup | measured **~80 s** (earlier guess 15-25 min) | `campaign` | `none` | TEST_ZERO |
...
| `contrib/perf/measure_dbcache_utxo.py` | `cache`/`rpc` | `cli_timer`+`debug_log` | YES (+ `getdbinfo`) |
| `qa/pull-tester/run-bitcoind-for-test.sh` | `init` | `debug_log` | YES |
| `./contrib/run-tests.sh` | `harness` | `none` | YES |
| `contrib/perf/performance-measurements.sh` | micros | `zcbench` | YES (+ archives for some benches) |
