...
**Finding (unchanged):** 22x`int64_t` always in RAM (~176 B/block); disk populate/serialize gated on `-zindex`; real RPC consumer in `rpc/blockchain.cpp`; **`nNotarizations` dead** (increment commented out).
