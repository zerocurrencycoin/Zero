...
| Full chain snap | macOS Application Support `zero/` | Live tip **2518018** (verified); `blocks/` ~10G + `chainstate` ~619M + `blocks/index` ~4.7G. Live tree may have **xattrs** (`com.apple.provenance`) with **no** on-disk `._*` (`find . -name '._*'` empty) -- macOS `tar` still emits AppleDouble into archives unless `COPYFILE_DISABLE=1`. Tip transplant needs `insightexplorer=1`. **Verified 2026-09-16, by full listing with sizes.** Neither archive contains any `._*` entry (2914 and 3033 entries, **0 AppleDouble each**), so the "COPYFILE_DISABLE" distinction the earlier note drew is **not what separates them**. Component sizes: |
| | | **`blk*.dat`+`rev*.dat`: 5.656 GB in both** (84 files each) -- chain data identical. **`chainstate/`: 0.604 GB in both** (338 vs 342 files) -- UTXO set identical. **`blocks/index/`: 4.694 GB / 2491 files vs 4.858 GB / 2606 files** -- the index carries the entire 0.15 GB archive delta. |
| | | **The index difference is LevelDB compaction, not growth.** Comparing `.ldb` generations: `-clean` has **202 files 812 lacks** and is **missing 87 that 812 has** (2397 shared, ranges 000534-016766 vs 000534-016987). LevelDB rewrites SSTables during compaction -- new generations appear, superseded ones are deleted -- so the later pack caught the index mid-reorganisation with more live files. Both are internally consistent: each has its own MANIFEST (`016662` vs a newer one) naming exactly the files it ships. |
| | | **How `-clean` was made.** `Perf.md` S0.16 records the command: `COPYFILE_DISABLE=1 tar -C "$PROD" -czf chainblocks812-clean.tgz blocks chainstate`. The evidence supports a straightforward re-pack of the *same live datadir* one hour later, not a different source: the newest mtime inside **both** archives is identical (`Jun 8 20:55`), so the block files are the same vintage; and the index files unique to each carry mtimes matching their pack time -- 812's extras are `Aug 12 22:42` (packed 22:56), `-clean`'s are `Aug 12 23:43` (packed 23:51). **A node was running against `$PROD` between the two packs**, and LevelDB compacted `blocks/index` in that hour. Neither archive contains `.sh` or any non-database file: both hold exactly 42 `blk*.dat`, 42 `rev*.dat`, `.ldb`, `LOG`, `LOG.old`, `MANIFEST-*` and one `.log` journal per database. |
| | | **Which to use: `-clean`.** Not because it is "cleaner" -- the name is misleading -- but because it is the `prep_lab_datadir.sh` default (`DEFAULT_ARCHIVE`), so using anything else silently diverges from every recorded lab run. For a *production* transplant neither is preferable on integrity grounds: the block data is identical and LevelDB will re-compact on first use either way. Tip transplant still needs `insightexplorer=1`. |
