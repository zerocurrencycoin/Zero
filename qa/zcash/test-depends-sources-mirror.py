...
def get_depends_sources_list():
    return [
        f for f in os.listdir(DEPENDS_SOURCES_DIR)
        if os.path.isfile(os.path.join(DEPENDS_SOURCES_DIR, f))
    ]
...
def main():
    for filename in get_depends_sources_list():
        ...
        if expected_size != server_size:
            print(
                "FAIL. On the server, %s is %d bytes, but locally it is %d bytes."
                % (filename, server_size, expected_size)
            )
            sys.exit(1)
