...
ZERO_CLI = './src/zero-cli'
USAGE = """
Requirements:
- find
- xz
- %s (edit ZERO_CLI in this script to alter the path)
- A running mainnet zerod using the default datadir with -txindex=1
...
def check_deps():
    if subprocess.call(['which', 'find', 'xz', ZERO_CLI], stdout=subprocess.PIPE):
        print(USAGE)
        sys.exit()
...
def create_benchmark_archive(blk_hash):
    blk = json.loads(subprocess.check_output([ZERO_CLI, 'getblock', blk_hash]))
    print('Height: %d' % blk['height'])
    print('Transactions: %d' % len(blk['tx']))

    os.mkdir('benchmark')
    with open('benchmark/block-%d.dat' % blk['height'], 'wb') as f:
        f.write(binascii.unhexlify(subprocess.check_output([ZERO_CLI, 'getblock', blk_hash, 'false']).strip()))

    txs = [json.loads(subprocess.check_output([ZERO_CLI, 'getrawtransaction', tx, '1'])
                     ) for tx in blk['tx']]
    ...
    for tx in bar(list(unique_inputs.keys())):
        rawtx = json.loads(subprocess.check_output([ZERO_CLI, 'getrawtransaction', tx, '1']))
        ...
        coins.extend(encode_varint(json.loads(
            subprocess.check_output([ZERO_CLI, 'getblockheader', rawtx['blockhash']])
            )['height']))
